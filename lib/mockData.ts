...
function getGameCountForDate(date: Date): number {
  ...
  if (dayOfWeek === 4) { // Thursday - TNT night
    return 6 + (dateHash % 4);
...
function getTeamPairing(date: Date, index: number): { home: Team; away: Team } {
  const seed = date.getFullYear() * 10000 + date.getMonth() * 100 + date.getDate() + index * 17;
 ...
 */
function determineGameStatus(gameTime: Date, now: Date): 'scheduled' | 'live' | 'final' {
 ...
 */
export function generateMockScores(gameId: string, status: 'live' | 'final'): GameScores {
 ...
 */
export function generateHistoricalGames(startDate: Date, endDate: Date): GameSafe[] {
