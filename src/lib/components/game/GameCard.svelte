...
<Card
	class="hover:shadow-lg transition-all duration-200 border-2 hover:border-blue-300 dark:hover:border-blue-700"
>
	<div class="p-6">
		...
		{#if !isRevealed && game.status !== 'scheduled'}
			...
			>
				...
				<div class="text-lg font-bold mb-2 text-gray-900 dark:text-white">
					Spoiler Protected
				</div>
		...
		{#if showPlayerStats && playerStats}
			<div
				class="mt-6 pt-6 border-t-2 border-gray-200 dark:border-gray-700 space-y-6"
			>
				<PlayerStatsTable
					teamName={game.awayTeam.name}
